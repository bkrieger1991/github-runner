...
# If TOKEN_FILE is set, read content into GITHUB_TOKEN
...
if [ -z "$GITHUB_TOKEN" ]; then
    echo "Error: no token set. Please set GITHUB_TOKEN or TOKEN_FILE environment variable."
    exit 1
fi

# Runner configuration and start
# We use --unattended and pass the parameters
./config.sh --url "${REPO_URL}" --token "${GITHUB_TOKEN}" --name "${RUNNER_NAME:-docker-runner}" --unattended --replace
